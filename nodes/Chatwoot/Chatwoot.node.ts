...
export class Chatwoot implements INodeType {
	...
	description: INodeTypeDescription = {
		...
		description:
			'Integração completa com Application API do Chatwoot + recursos custom chatwoot-x',
		...
		properties: [
			// ─────────────────────── RESOURCE ───────────────────────
			{
				...
				options: [
					...
					{ name: 'Account Agent Bot', value: 'accountAgentBot' },
					...
					{ name: 'Flow (Chatwoot-X)', value: 'flow' },
					{ name: 'Flow Folder (Chatwoot-X)', value: 'flowFolder' },
					{ name: 'Flow Run (Chatwoot-X)', value: 'flowRun' },
					{ name: 'Help Center', value: 'helpCenter' },
					{ name: 'Inbox', value: 'inbox' },
					{ name: 'Inbox Member', value: 'inboxMember' },
					{ name: 'Integration', value: 'integration' },
					{ name: 'Label', value: 'label' },
					{ name: 'Lifecycle Stage (Chatwoot-X)', value: 'lifecycleStage' },
			...
			// ═══════════════════════════════════════════════════════════════════
			{
				...
				options: [
					...
					},
					{
						...
						value: 'add',
						action: 'Add an agent',
						routing: { request: { method: 'POST', url: '=/api/v1/accounts/{{$credentials.accountId}}/agents' } },
					},
					{
						name: 'Update',
						value: 'update',
						action: 'Update an agent',
						routing: { request: { method: 'PATCH', url: '=/api/v1/accounts/{{$credentials.accountId}}/agents/{{$parameter["agentId"]}}' } },
					},
					{
						name: 'Remove',
						value: 'remove',
						action: 'Remove an agent',
						routing: { request: { method: 'DELETE', url: '=/api/v1/accounts/{{$credentials.accountId}}/agents/{{$parameter["agentId"]}}' } },
					},
				],
				default: 'list',
			},
			{
				...
				required: true,
				default: '',
				displayOptions: { show: { resource: ['agent'], operation: ['update', 'remove'] } },
			},
			{
				displayName: 'User ID',
				name: 'user_id',
				...
				required: true,
				default: '',
				displayOptions: { show: { resource: ['agent'], operation: ['add'] } },
			...
			},
			{
				...
				displayOptions: { show: { resource: ['agent'], operation: ['add', 'update'] } },
			...
			// ═══════════════════════════════════════════════════════════════════
			{
				...
				options: [
					...
					},
					{
						...
						routing: { request: { method: 'PATCH', url: '=/api/v1/accounts/{{$credentials.accountId}}/custom_attribute_definitions/{{$parameter["customAttributeId"]}}' } },
					},
					{
						name: 'Delete',
						value: 'delete',
						action: 'Delete a custom attribute',
						routing: { request: { method: 'DELETE', url: '=/api/v1/accounts/{{$credentials.accountId}}/custom_attribute_definitions/{{$parameter["customAttributeId"]}}' } },
					},
				],
				default: 'list',
			},
			{
				...
				required: true,
				default: '',
				displayOptions: { show: { resource: ['customAttribute'], operation: ['get', 'update', 'delete'] } },
			},
			{
			...
			},
			{
				...
				displayOptions: { show: { resource: ['customAttribute'], operation: ['create', 'update'] } },
				routing: { send: { type: 'body' } },
				description: 'Attribute_display_type: 0=Text 1=Number 2=Currency 3=Percent 4=Link 5=Date 6=List 7=Checkbox. attribute_model: 0=Conversation 1=Contact.',
			},
			...
			// ═══════════════════════════════════════════════════════════════════
			{
				...
				options: [
					...
					},
					{
						...
						routing: { request: { method: 'POST', url: '=/api/v1/accounts/{{$credentials.accountId}}/inboxes' } },
					},
					{
						name: 'Get',
						value: 'get',
						action: 'Get an inbox',
						routing: { request: { method: 'GET', url: '=/api/v1/accounts/{{$credentials.accountId}}/inboxes/{{$parameter["inboxId"]}}' } },
					},
					{
						name: 'Update',
						value: 'update',
						action: 'Update an inbox',
						routing: { request: { method: 'PATCH', url: '=/api/v1/accounts/{{$credentials.accountId}}/inboxes/{{$parameter["inboxId"]}}' } },
					},
					{
						name: 'Get Agent Bot',
						value: 'getAgentBot',
						action: 'Get inbox agent bot',
						routing: { request: { method: 'GET', url: '=/api/v1/accounts/{{$credentials.accountId}}/inboxes/{{$parameter["inboxId"]}}/agent_bot' } },
					},
					{
						name: 'Set Agent Bot',
						value: 'setAgentBot',
						action: 'Set inbox agent bot',
						routing: { request: { method: 'POST', url: '=/api/v1/accounts/{{$credentials.accountId}}/inboxes/{{$parameter["inboxId"]}}/set_agent_bot' } },
					},
				],
				default: 'list',
			},
			{
				...
				required: true,
				default: '',
				displayOptions: { show: { resource: ['inbox'], operation: ['get', 'update', 'getAgentBot', 'setAgentBot'] } },
			},
			{
				displayName: 'Body (JSON)',
				name: 'body',
				...
				routing: { send: { type: 'body' } },
				description: 'Payload completo. channel.type aceita: WebWidget, Api, Email, FacebookPage, TwilioSms, WhatsApp, Telegram, Line, Sms, Whatsapp etc.',
			},
			{
				...
				routing: { send: { type: 'body' } },
				description: 'Campos a atualizar. Suporta name, greeting_enabled, greeting_message, working_hours_enabled, allow_messages_after_resolved, etc.',
			},
			{
				displayName: 'Agent Bot ID',
				name: 'agent_bot',
				type: 'string',
				default: '',
				displayOptions: { show: { resource: ['inbox'], operation: ['setAgentBot'] } },
				routing: { send: { type: 'body', property: 'agent_bot' } },
				description: 'Deixe vazio pra desligar o bot',
			},
			...
			// ═══════════════════════════════════════════════════════════════════
			{
				...
				options: [
					...
					},
					{
						...
						value: 'get',
						action: 'Get a label',
						routing: { request: { method: 'GET', url: '=/api/v1/accounts/{{$credentials.accountId}}/labels/{{$parameter["labelId"]}}' } },
					},
					{
						name: 'Update',
						value: 'update',
			...
			},
			{
				...
				required: true,
				default: '',
				displayOptions: { show: { resource: ['label'], operation: ['get', 'update', 'delete'] } },
			},
			{
				displayName: 'Title',
				name: 'title',
			...
			},
			{
				...
				options: [
					...
					{ displayName: 'Show on Sidebar', name: 'show_on_sidebar', type: 'boolean', default: true, routing: { send: { type: 'body', property: 'show_on_sidebar' } } },
				],
			},
			{
				displayName: 'Update Fields',
				...
				options: [
					{ displayName: 'Title', name: 'title', type: 'string', default: '', routing: { send: { type: 'body', property: 'title' } } },
					{ displayName: 'Description', name: 'description', type: 'string', default: '', routing: { send: { type: 'body', property: 'description' } } },
					{ displayName: 'Color', name: 'color', type: 'color', default: '#1F93FF', routing: { send: { type: 'body', property: 'color' } } },
					{ displayName: 'Show on Sidebar', name: 'show_on_sidebar', type: 'boolean', default: true, routing: { send: { type: 'body', property: 'show_on_sidebar' } } },
				],
			...
			},
			{
				...
				default: 'outgoing',
				options: [
					{ name: 'Outgoing (Agent → Contact)', value: 'outgoing' },
					{ name: 'Incoming (Contact → Agent)', value: 'incoming' },
				],
				displayOptions: { show: { resource: ['message'], operation: ['create'] } },
			...
			},
			{
				...
				description: 'Atributos extras pro content_type (ex: items pra input_select, fields pra form)',
			},
			{
				...
				description: 'Pra WhatsApp Cloud — { name, category, language, processed_params: { body, header } }',
			},
			{
			...
			// ═══════════════════════════════════════════════════════════════════
			{
				...
				options: [
					...
					},
					{
						...
						routing: { request: { method: 'POST', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams' } },
					},
					{
						name: 'Get',
						value: 'get',
						action: 'Get a team',
						routing: { request: { method: 'GET', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}' } },
					},
					{
						name: 'Update',
						value: 'update',
						action: 'Update a team',
						routing: { request: { method: 'PATCH', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}' } },
					},
					{
						name: 'Delete',
						value: 'delete',
						action: 'Delete a team',
						routing: { request: { method: 'DELETE', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}' } },
					},
					{
						name: 'List Members',
						value: 'listMembers',
						action: 'List team members',
						routing: { request: { method: 'GET', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}/team_members' } },
					},
					{
						name: 'Add Members',
						value: 'addMembers',
						action: 'Add team members',
						routing: { request: { method: 'POST', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}/team_members' } },
					},
					{
						name: 'Update Members',
						value: 'updateMembers',
						action: 'Replace team members',
						routing: { request: { method: 'PATCH', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}/team_members' } },
					},
					{
						name: 'Remove Members',
						value: 'removeMembers',
						action: 'Remove team members',
						routing: { request: { method: 'DELETE', url: '=/api/v1/accounts/{{$credentials.accountId}}/teams/{{$parameter["teamId"]}}/team_members' } },
					},
				],
				default: 'list',
			},
			{
				...
				required: true,
				default: '',
				displayOptions: { show: { resource: ['team'], operation: ['get', 'update', 'delete', 'listMembers', 'addMembers', 'updateMembers', 'removeMembers'] } },
			},
			{
				displayName: 'Name',
				name: 'name',
			...
			},
			{
				...
				options: [
					{ displayName: 'Description', name: 'description', type: 'string', default: '', routing: { send: { type: 'body', property: 'description' } } },
					{ displayName: 'Allow Auto Assign', name: 'allow_auto_assign', type: 'boolean', default: true, routing: { send: { type: 'body', property: 'allow_auto_assign' } } },
				],
			},
			{
				displayName: 'Update Fields',
				name: 'updateTeamFields',
				type: 'collection',
				placeholder: 'Add field',
				default: {},
			...
			},
			{
				...
				default: '',
				placeholder: '1,2,3',
				displayOptions: { show: { resource: ['team'], operation: ['addMembers', 'updateMembers', 'removeMembers'] } },
				routing: { send: { type: 'body', property: 'user_ids', value: '={{$value.split(",").map(s => Number(s.trim())).filter(Boolean)}}' } },
				description: 'IDs de usuários separados por vírgula. PATCH substitui toda a lista.',
			},
